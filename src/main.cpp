...
void setup(){
	...
	digitalWrite(bus_pin[1],LOW);
	if(!(digitalRead(pu_pin[2])||digitalRead(pu_pin[3])))hidp.clearBondStoreData();
	digitalWrite(bus_pin[1],HIGH);
...
void loop(){
		...
		for(uint8_t j=0;j<4;j++,p++){
			...
			if((btn_p^btn)&(1<<p)){
				...
				switch(p){
					...
					case 1:{if(x)kbd.press(KEYCODE_LEFT_SHIFT,KEYCODE_MOD_LEFT_SHIFT);else kbd.release(KEYCODE_LEFT_SHIFT,KEYCODE_MOD_LEFT_SHIFT);break;}// B
					...
					case 5:{break;}// +
					case 6:{if(x)mouse.move(0,0,255);break;}// L
					case 7:{if(x)mouse.move(0,0,-256);break;}// R
	...
	if(st[0]||st[1])mouse.move(st[0],st[1]);
